 ...
 * 成绩比例*/
@Controller
public class ChengjibiliController extends BaseController {
    @Autowired
    private ChengjibiliMapper dao;
    @Autowired
    private ChengjibiliService service;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/chengjibili_list")
    public String list()
    {
        ...
        Example example = new Example(Chengjibili.class); //  创建一个扩展搜索类
        ...
        List<Chengjibili> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
    ...
    @RequestMapping("/chengjibili_add")
    public String add()
    {
        _var = new LinkedHashMap(); // 重置数据
...
//        assign("zhuanyeList" , new CommDAO().select("SELECT * FROM zhuanye ORDER BY id desc"));
    ...
    @RequestMapping("/chengjibili_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Chengjibili mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
     ...
     * @return
     */
    @RequestMapping("/chengjibiliinsert")
    public String insert()
    {
        _var = new LinkedHashMap(); // 重置数据
        String tmp="";
        Chengjibili post = new  Chengjibili();  // 创建实体类
        // 设置前台提交上来的数据到实体类中
        ...
        post.setJiaoshipingjiachengji(Request.get("jiaoshipingjiachengji"));

        post.setQiangdawentichengji(Request.get("qiangdawentichengji"));
     ...
     * @return
     */
    @RequestMapping("/chengjibiliupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Chengjibili post = new Chengjibili();
        // 将前台表单数据填充到实体类
        if(!Request.get("kechengid").equals(""))
     ...
     *  删除
     */
    @RequestMapping("/chengjibili_delete")
    public String delete()
    {
        ...
        HashMap map = Query.make("chengjibili").find(id);
