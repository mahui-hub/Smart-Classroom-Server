 ...
 * 学生互评 */
@Controller
public class XueshenghupingController extends BaseController
{
    @Autowired
    private XueshenghupingMapper dao;
    @Autowired
    private XueshenghupingService service;
    ...
    public String list()
    {
        ...
        Example example = new Example(Xueshenghuping.class); //  创建一个扩展搜索类
...
//        }
     ...
     * 互评人列表
     */
    @RequestMapping("/xueshenghuping_list_hupingren")
    public String listhupingren()
    {
        // 检测是否有登录，没登录则跳转到登录页面
        if(!checkLogin()){
            return showError("尚未登录" , "./login.do");
        ...
        Example example = new Example(Xueshenghuping.class);  //  创建一个扩展搜索类
        Example.Criteria criteria = example.createCriteria();           // 创建一个扩展搜索条件类
        // 初始化一个条件，条件为：互评人=当前登录用户
        String where = " hupingren='"+request.getSession().getAttribute("username")+"' ";
        where += getWhere();
        ...
        page = Math.max(1 , page); // 注释同list
            ...
            List<Xueshenghuping> list = service.selectPageExample(example , page , pagesize);
                    assign("totalCount" , request.getAttribute("totalCount"));
        assign("list" , list);
        ...
        @RequestMapping("/xueshenghuping_add")
    public String add()
    {
        _var = new LinkedHashMap(); // 重置数据
    ...
    @RequestMapping("/xueshenghuping_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Xueshenghuping mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
     ...
     * @return
     */
    @RequestMapping("/xueshenghupinginsert")
...
//        post.setXueshengid(Request.getInt("xueshengid"));
    ...
    * @return
    */
    @RequestMapping("/xueshenghupingupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Xueshenghuping post = new Xueshenghuping();
        // 将前台表单数据填充到实体类
        if(!Request.get("xuehao").equals(""))
        post.setXuehao(Request.get("xuehao"));
        if(!Request.get("banjiid").equals(""))
...
//            post.setChishu(Integer.valueOf(Request.get("chishu")));
     ...
     *  后台详情
     */
    @RequestMapping("/xueshenghuping_detail")
    public String detail()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Xueshenghuping map = service.find(id);  // 根据前台url 参数中的id获取行数据
    ...
    *  删除
    */
    @RequestMapping("/xueshenghuping_delete")
    public String delete()
    {
        ...
        HashMap map = Query.make("xueshenghuping").find(id);
