...
public class BanjiController extends BaseController
{
    ...
    @Autowired
    private BanjiService service;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/banji_list")
    public String list()
    {
        ...
        Example example = new Example(Banji.class); //  创建一个扩展搜索类
        ...
        List<Banji> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
    ...
    @RequestMapping("/banji_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Banji mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
    ...
    */
    ...
    public String delete()
    {
        ...
        HashMap map = Query.make("banji").find(id);
