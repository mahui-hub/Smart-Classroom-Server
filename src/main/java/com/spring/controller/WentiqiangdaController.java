 ...
 * 问题抢答 */
@Controller
public class WentiqiangdaController extends BaseController
{
    @Autowired
    private WentiqiangdaMapper dao;
    @Autowired
    private WentiqiangdaService service;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/wentiqiangda_list")
    public String list()
    {
        ...
        Example example = new Example(Wentiqiangda.class); //  创建一个扩展搜索类
        ...
        List<Wentiqiangda> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
     ...
     * 发布人列表
     */
    @RequestMapping("/wentiqiangda_list_faburen")
    public String listfaburen()
    {
        ...
        page = Math.max(1 , page); // 注释同list
            ...
            List<Wentiqiangda> list = service.selectPageExample(example , page , pagesize);
                    assign("totalCount" , request.getAttribute("totalCount"));
        assign("list" , list);
    ...
    *
    */
    @RequestMapping("/wentiqiangdalist")
    public String index()
    {
            String order = Request.get("order" , "id");
        String sort  = Request.get("sort" , "desc");

        Example example = new Example(Wentiqiangda.class);
        ...
        @RequestMapping("/wentiqiangda_add")
    public String add()
    {
        _var = new LinkedHashMap(); // 重置数据
    ...
    @RequestMapping("/wentiqiangda_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Wentiqiangda mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
     ...
     * @return
     */
    @RequestMapping("/wentiqiangdainsert")
    public String insert()
    {
        ...
        post.setQiangdarenshu(Request.getInt("qiangdarenshu"));

        post.setYiqiangrenshu(Request.getInt("yiqiangrenshu"));
    ...
    * @return
    */
    @RequestMapping("/wentiqiangdaupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Wentiqiangda post = new Wentiqiangda();
        // 将前台表单数据填充到实体类
        if(!Request.get("bianhao").equals(""))
        post.setBianhao(Request.get("bianhao"));
                if(!Request.get("biaoti").equals(""))
        ...
        post.setQiangdarenshu(Request.getInt("qiangdarenshu"));
            if(!Request.get("yiqiangrenshu").equals(""))
        post.setYiqiangrenshu(Request.getInt("yiqiangrenshu"));
     ...
     *  后台详情
     */
    @RequestMapping("/wentiqiangda_detail")
    public String detail()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Wentiqiangda map = service.find(id);  // 根据前台url 参数中的id获取行数据
     ...
     *  前台详情
     */
    @RequestMapping("/wentiqiangdadetail")
    public String detailweb()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Wentiqiangda map = service.find(id);
    ...
    *  删除
    */
    @RequestMapping("/wentiqiangda_delete")
    public String delete()
    {
        ...
        HashMap map = Query.make("wentiqiangda").find(id);
