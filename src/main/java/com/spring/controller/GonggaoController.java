...
public class GonggaoController extends BaseController
{
    ...
    public String list()
    {
        ...
        List<Gonggao> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
     ...
     * 发布人列表
     */
    @RequestMapping("/gonggao_list_faburen")
    public String listfaburen()
    {
        ...
        page = Math.max(1 , page); // 注释同list
            ...
            List<Gonggao> list = service.selectPageExample(example , page , pagesize);
                    assign("totalCount" , request.getAttribute("totalCount"));
        assign("list" , list);
    ...
    */
    ...
    public String detailweb()
    {
        ...
        Gonggao map = service.find(id);
                            assign("prev" , new CommDAO().find("SELECT * FROM gonggao WHERE id<'"+request.getParameter("id")+"' ORDER BY id desc"));
            assign("next" , new CommDAO().find("SELECT * FROM gonggao WHERE id>'"+request.getParameter("id")+"' ORDER BY id desc"));        
