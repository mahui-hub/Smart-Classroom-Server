...
public class XueyuanController extends BaseController{
    ...
    @Autowired
    private XueyuanService service;
    /**
     *  后台列表页
     *
     */
    @RequestMapping("/xueyuan_list")
    public String list()
    {
        ...
        Example example = new Example(Xueyuan.class); //  创建一个扩展搜索类
        ...
        List<Xueyuan> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
...
//        assign("zhuanyeList" , new CommDAO().select("SELECT * FROM zhuanye ORDER BY id desc"));
    ...
    @RequestMapping("/xueyuan_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Xueyuan mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
...
//        assign("zhuanyeList" , new CommDAO().select("SELECT * FROM zhuanye ORDER BY id desc"));
     ...
     * @return
     */
    @RequestMapping("/xueyuaninsert")
    public String insert()
    {
        _var = new LinkedHashMap(); // 重置数据
        String tmp="";
        Xueyuan post = new Xueyuan();  // 创建实体类
        // 设置前台提交上来的数据到实体类中
     ...
     * @return
     */
    @RequestMapping("/xueyuanupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Xueyuan post = new Xueyuan();
        // 将前台表单数据填充到实体类
        if(!Request.get("xueyuan").equals(""))
            post.setXueyuan(Request.get("xueyuan"));
